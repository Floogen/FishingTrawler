    ...
    public class FishingTrawler : Mod
    {
        ...
        private void OnGameLaunched(object sender, GameLaunchedEventArgs e)
        {
            ...
            if (Helper.ModRegistry.IsLoaded("spacechase0.GenericModConfigMenu") && apiManager.HookIntoGMCM(Helper))
            {
                ...
                configAPI.AddNumberOption(ModManifest, () => config.hullEventFrequencyLower, (val) => config.hullEventFrequencyLower = val, () => i18n.Get("config.option.event_frequency_lower.name"), () => i18n.Get("config.option.hull.event_frequency_lower.description"), 1, 15);
                configAPI.AddNumberOption(ModManifest, () => config.hullEventFrequencyUpper, (val) => config.hullEventFrequencyUpper = val, () => i18n.Get("config.option.event_frequency_upper.name"), () => i18n.Get("config.option.hull.event_frequency_upper.description"), 1, 15);
                configAPI.AddNumberOption(ModManifest, () => config.netEventFrequencyLower, (val) => config.netEventFrequencyLower = val, () => i18n.Get("config.option.event_frequency_lower.name"), () => i18n.Get("config.option.net.event_frequency_lower.description"), 1, 15);
                configAPI.AddNumberOption(ModManifest, () => config.netEventFrequencyUpper, (val) => config.netEventFrequencyUpper = val, () => i18n.Get("config.option.event_frequency_upper.name"), () => i18n.Get("config.option.net.event_frequency_upper.description"), 1, 15);
